...
void StackFrame::run (string filename) {
    ...
    while (!Instructions.eof()) {
            ...
            if (wordsTracking [0] == "iadd") { // Lenh 1
                if (Stack.maxIndex() > 2) {
                    if (max ((int) Stack.topDataType(), (int) Stack.belowTopDataType()) == 0) {
                        ...
                        Instructions.close();
                        Stack.destroyStack();
                        LocalVariableSpace.destroyTree();
        ...
        else
        {
            ...
            {
        if (LocalVariableSpace.searchKey(wordsTracking[1]) == NULL) {
            delete[] wordsTracking;
            Instructions.close();
            Stack.destroyStack();
                    LocalVariableSpace.destroyTree();
            ...
            if (LocalVariableSpace.searchKey(wordsTracking[1])->dataType == 0) {
                    if(!Stack.isFull()) {
                        Stack.push((int)LocalVariableSpace.searchKey(wordsTracking[1])->dataValue, 0);
                    }
                    else {
                        delete[] wordsTracking;
                        Instructions.close();
        ...
        if (LocalVariableSpace.searchKey(wordsTracking[1]) == NULL) {
            ...
            if (LocalVariableSpace.searchKey(wordsTracking[1])->dataType == 1) {
                    if(!Stack.isFull()) {
                        Stack.push((float)LocalVariableSpace.searchKey(wordsTracking[1])->dataValue, 1);
                    }
                    else {
                        delete[] wordsTracking;
                        Instructions.close();
            ...
            {
        if(LocalVariableSpace.getSize() == 256) {
            delete[] wordsTracking;
            Instructions.close();
            Stack.destroyStack();
                    LocalVariableSpace.destroyTree();
            throw LocalSpaceFull(lineNum);
        }
        else {
            if(!Stack.isEmpty())
            {
                ...
                {
                    topDataOfStack = Stack.topDataValue(); Stack.pop();
                    LocalVariableSpace.insert(wordsTracking[1], (int)topDataOfStack, 0);
                }
                else
                {
                    delete[] wordsTracking;
        ...
        if(LocalVariableSpace.getSize() == 256) {
            ...
            if(!Stack.isEmpty())
            {
                ...
                {
                    topDataOfStack = Stack.topDataValue(); Stack.pop();
                    LocalVariableSpace.insert(wordsTracking[1], (float)topDataOfStack, 1);
                }
                else
                {
                    delete[] wordsTracking;
        ...
        else if (wordsTracking[0] == "val") { // Lenh 32
            if (LocalVariableSpace.searchKey(wordsTracking[1]) == NULL) {
                delete[] wordsTracking;
                Instructions.close();
                Stack.destroyStack();
                ...
                if (LocalVariableSpace.searchKey(wordsTracking[1])->dataType == 0) {
                   cout << (int) LocalVariableSpace.searchKey(wordsTracking[1])->dataValue << "\n";
                }
               else {
                   cout << (float) LocalVariableSpace.searchKey(wordsTracking[1])->dataValue << "\n";
               }
            }
        }    
            ...
            if (LocalVariableSpace.searchKey(wordsTracking[1]) == NULL) {
                ...
                if (LocalVariableSpace.findParent(wordsTracking[1]) != NULL) {
                    cout << LocalVariableSpace.findParent(wordsTracking[1])->key << "\n";
                }
                else cout<< "null" << endl;
            }
        }
